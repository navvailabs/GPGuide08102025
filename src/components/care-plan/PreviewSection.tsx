...
const ParsedTableToDocument = ({ tableHtml }: { tableHtml: string }) => {
    // Define a colorful array of divider colors
    const dividerColors = useMemo(() => [
        'border-cyan-400/50',
        'border-rose-400/50',
        'border-amber-400/50',
    ...
    const parsedContent = useMemo(() => {
        ...
        // 3. Process each data row into a visually distinct section.
        const documentSections = dataRows.map((row, rowIndex) => {
                ...
                if (textContent) {
                    return (
                        <div key={`section-title-${rowIndex}`}>
                            {rowIndex > 0 && <hr className={cn("my-8 border-dashed", dividerColors[rowIndex % dividerColors.length])} />}
                            <h4 className="text-xl font-bold text-white mb-4" dangerouslySetInnerHTML={{ __html: cells[0].innerHTML }} />
                        </div>
                    );
                }
                return null;
            ...
            // Handle standard data rows by creating a group of pairs
            const rowPairs = cells.map((cell, cellIndex) => {
                ...
                if (header && textContent) {
                    return (
                        <div key={`pair-${rowIndex}-${cellIndex}`}>
                            <p className="prose prose-sm prose-invert max-w-none text-gray-300 prose-p:my-0 prose-ul:my-0 prose-li:my-1">
                                <strong className="text-gray-100 font-semibold" dangerouslySetInnerHTML={{ __html: header.trim().replace(/:$/, '') + ':' }} />
                                {' '}
            ...
            if (rowPairs.length === 0) return null;

            return (
                <React.Fragment key={`row-group-${rowIndex}`}>
                    {rowIndex > 0 && <hr className={cn("my-6", dividerColors[rowIndex % dividerColors.length])} />}
            ...
            const fallbackSections = dataRows.map((row, rowIndex) => {
                ...
                if (cells.length === 2) {
                    ...
                    if (cells[0].textContent?.trim() || cells[1].textContent?.trim()) {
                        return (
                            <React.Fragment key={`fallback-group-${rowIndex}`}>
                                {rowIndex > 0 && <hr className={cn("my-6", dividerColors[rowIndex % dividerColors.length])} />}
                                <div className="space-y-4">
                                    <div key={`fallback-pair-${rowIndex}`}>
                                        <p className="prose prose-sm prose-invert max-w-none text-gray-300 prose-p:my-0 prose-ul:my-0 prose-li:my-1">
                                            <strong className="text-gray-100 font-semibold" dangerouslySetInnerHTML={{ __html: label.trim().replace(/:$/, '') + ':' }} />
                                            {' '}
