...
const MEDDAssistTool = ({ medicationInput, setMedicationInput, result, setResult }: MEDDAssistToolProps) => {
    ...
    const getRiskLevel = (medd: number) => {
        if (medd >= 90) return { text: 'High Risk', color: 'text-red-400', bg: 'bg-red-400/10', border: 'border-red-400/30' };
        if (medd >= 50) return { text: 'Increased Risk', color: 'text-amber-400', bg: 'bg-amber-400/10', border: 'border-amber-400/30' };
        return { text: 'Lower Risk', color: 'text-green-400', bg: 'bg-green-400/10', border: 'border-green-400/30' };
    };
    ...
    return (
        ...
        >
            <motion.div variants={sectionVariants} className="mb-10 text-center">
                <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-white">Morphine Equivalent Daily Dose (MEDD) Assist</h2>
                <p className="mt-3 text-base text-gray-400">Calculate total MEDD using Australian conversion standards.</p>
            </motion.div>
                ...
                <motion.section variants={sectionVariants}>
                    <div className="bg-[#1F2023]/70 backdrop-blur-lg border border-white/10 rounded-2xl p-6 shadow-2xl">
                        ...
                        <textarea
                            ...
                            className="form-textarea w-full rounded-lg border-white/20 bg-black/20 focus:ring-2 focus:ring-primary focus:border-primary placeholder:text-gray-500 text-white font-mono text-sm"
                            rows={5}
                        />
                    </div>
                </motion.section>
                ...
                {result && (
                    <motion.div variants={sectionVariants} className="border-t border-white/10 pt-8 mt-12">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-2xl font-bold text-white">Calculation Result</h3>
                            <button
                                onClick={handleCopy}
                        ...
                        <div className="bg-[#1F2023]/70 backdrop-blur-lg border border-white/10 rounded-2xl p-6 shadow-2xl space-y-6">
                            ...
                            <div>
                                <h4 className="text-lg font-semibold mb-3 text-white">Breakdown</h4>
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm text-left">
                                        <thead className="bg-white/5">
                                            <tr>
                                                <th className="p-3 font-medium border-b border-white/10">Medication</th>
                                                <th className="p-3 font-medium border-b border-white/10 text-right">Calculated MEDD (mg)</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {result.breakdown.map((item, index) => (
                                                <tr key={index} className="border-b border-white/10 last:border-b-0">
                                                    <td className="p-3">
                                                        <p className="text-gray-300">{item.originalLine}</p>
                                                        {item.error && <p className="text-xs text-red-400 mt-1">{item.error}</p>}
                                                    </td>
                                                    <td className="p-3 text-right font-mono text-gray-300">{item.error ? '-' : item.medd.toFixed(1)}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                            ...
                            <div className="text-xs text-gray-500 pt-4 border-t border-white/10">
                                ...
                                <a href="https://www.anzca.edu.au/resources/faculty-of-pain-medicine" target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex items-center gap-1.5 text-sky-400 hover:text-sky-300 transition-colors">
